...
However, this is not the entire truth-table. We have not considered the unknown state. How should the table expand to include `0` inputs?
...
1. Permutation (e.g. `roll-up(a)`)
2. Non-bijective transformation (e.g. `is_plus(a)`)
3. The constants `+`, `0`, `-`

Permutations: The unary operator `roll_up(a)` adds one to `a` and rolls around to `-` if the result is 2. The inverse is `roll_down(a)`. Applying either operation three times returns us back to `a`.

| `a` | `roll_up(a)` | `roll_up(roll_up(a))` | `roll_up(roll_up(roll_up(a)))` |
| --- | ------------ | --------------------- | ------------------------------ |
| `-` | `0`          | `+`                   | `-`                            |
| `0` | `+`          | `-`                   | `0`                            |
| `+` | `-`          | `0`                   | `+`                            |

 We can therefore define `roll_down(a)` as `roll_up(roll_up(a))`. (Or we can define `roll_up` as `roll_down` applied twice.)

> [!note] Formulas
> - `roll_up(a)` = `roll_down(roll_down(a))`
> - `roll_down(a)` = `roll_up(roll_up(a))`
> - `roll_up(roll_up(roll_up(a)))` = `roll_down(roll_down(roll_down(a)))` = `a`

Non-bijective transformations: The unary operator `is_plus(a)` is `+` if `a` is `+`, and `-` otherwise. We can define `is_minus` and `is_zero` from `is_plus` and `roll_up`/`roll_down`:

> [!note] Formulas
> - `is_zero(a)` = `is_plus(roll_up(a))` = `is_minus(roll_down(a))`
> - `is_minus(a)` = `is_plus(roll_down(a))` = `is_zero(roll_up(a))`

The `roll_up` and `is_plus` functions are enough to generate any unary gate you can imagine. To generate any arbitrary binary operator, you can use the [SEL function](./sel-function.md).
